...
WITH ad_squad as ( SELECT 
    -- ad squad, media buy name , campaign id and cost model
        ...
        ROW_NUMBER() OVER(PARTITION BY JSON_EXTRACT_SCALAR(data, "$.id"),JSON_EXTRACT_SCALAR(data, "$.campaign_id"),JSON_EXTRACT_SCALAR(data, "$.name"),JSON_EXTRACT_SCALAR(data, "$.type") ORDER BY _sdc_extracted_at DESC ) as row_num
